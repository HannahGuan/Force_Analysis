...
class Force_analysis:
    ...
    def calibrate(self, dataf, base, keyboard): # "self" is a special keyword used in class objects in python
        """
        ...
        """
        dataf['force_F']  = (dataf['force_F']  - key_f[1])
        dataf['force_G']  = (dataf['force_G']  - key_g[1])
        dataf['force_J']  = (dataf['force_J']  - key_j[1])
        dataf['force_K']  = (dataf['force_K']  - key_k[1])
        dataf['force_N']  = (dataf['force_N']  - key_n[1])

        dataf['force_F']  = (dataf['force_F'])/key_f[0]
        dataf['force_G']  = (dataf['force_G'])/key_g[0]
        dataf['force_J']  = (dataf['force_J'])/key_j[0]
        dataf['force_K']  = (dataf['force_K'])/key_k[0]
        dataf['force_N']  = (dataf['force_N'])/key_n[0]
    ...
    def col_read(self):
        ...
        base = dataf.iloc[:500, 0:5].mean() #baseline

        #reduce the trials
        start = self.ps_data['Distractor_onset'][self.ps_data.index[0]]
        ind = 0

        # why I write this loop: to delete the lines we do not need (the lines far away 
            #from the target onset) so that we can improve the time efficiency
        ...
        dataf=dataf[ind-100:]
        self.calibrate(dataf, base, self.ps_data["Computer"][self.ps_data.index[0]])
...
def folder_select(psych_folder = "Data", force_folder = "Forcefiles"):
    ...
    time_x = np.arange(0, 1500, 1)
    print(prime_dict["pc_prev_cong"])
    print(prime_dict["pc_prev_incon"])
